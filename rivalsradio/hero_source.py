...
- ``NativeHeroSource``: receives hero/match data from the companion native
  Overwolf app over localhost HTTP (the default).
- ``GepHeroSource``: launches the Overwolf ow-electron bridge as a child
  process and reads exact hero changes from a file it tails.
    ...
    def _read(self, on_hero: HeroFn, on_log: LogFn,
        ...
        try:
            while not self._stop.is_set():
                ...
                elif mtype in ("match", "stats", "needs_admin", "game") and on_event:
                    on_event(msg)
        finally:
            if fh:
                fh.close()
